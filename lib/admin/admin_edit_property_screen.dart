...
class _AdminEditPropertyScreenState extends State<AdminEditPropertyScreen> {
  ...
  bool _available = true, _loading = false, _initialising = true;
  List<String> _images = [];
  ...
  Future<void> _loadProperty() async {
    if (widget.propertyId != null) {
      try {
        final p = await _service.getPropertyById(widget.propertyId!);
        if (p != null) _fill(p);
  ...
  Widget build(BuildContext context) {
    return Scaffold(appBar: AppBar(title: Text(widget.propertyId == null ? 'Add Property' : 'Edit Property')),
      body: _initialising ? const Center(child:CircularProgressIndicator()) : Form(key:_formKey, child: ListView(padding: const EdgeInsets.all(18), children: [
        _field(_title,'Title'), _field(_address,'Address'), _field(_postcode,'Postcode'),
        _counter('Bedrooms',_bedrooms,(v)=>setState(()=>_bedrooms=v)), _counter('Bathrooms',_bathrooms,(v)=>setState(()=>_bathrooms=v)), const SizedBox(height:8),
        _field(_rent,'Monthly rent (£)',type:TextInputType.number,validator:(v)=>double.tryParse(v?.trim()??'')==null?'Enter a valid amount':null),
        _field(_deposit,'Deposit (£) (optional)',type:TextInputType.number,validator:(v)=>v!.trim().isEmpty||double.tryParse(v.trim())!=null?null:'Enter a valid amount'),
